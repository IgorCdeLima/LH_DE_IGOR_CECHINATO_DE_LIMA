...
class TableLoader:
    ...
    def agencias_table(self, address):
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                if Path(address).exists():
                    ...
                    if (agencias) is True:
                        for chunk in pd.read_csv(address, chunksize=self.CHUNKSIZE):
    ...
    def clientes_table(self, address):
    ...
    def colaborador_agencia_table(self, address):
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                if Path(address).exists():
                    ...
                    if (colaborador_agencia) is True:
                        for chunk in pd.read_csv(address, chunksize=self.CHUNKSIZE):
                            chunk.columns = chunk.columns.str.strip()  
                            for index, row in chunk.iterrows():
                                data = {
    ...
    def colaboradores_table(self, address):
    ...
    def propostas_credito_table(self, address):
